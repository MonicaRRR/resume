...
async def suggest_resume_patch(
    ...
    prompt = _prompt(
        ...
        constraints=[
            ...
            "工作经历与项目经历坚持 STAR：每条改写都要交代情境/任务、个人行动和结果；结果优先保留数字、规模、效率、质量或上线影响。",
            "能拆成清晰要点就拆成 2–4 条 bullets：背景/任务、关键行动、结果不要揉成一条难读长段；同一经历最多保留必要要点。",
            ...
            "注意版面密度：正文 bullet 尽量写到一行的大部分宽度（中文通常约 25–60 字，按实际模板换行）；避免只有几个字的短尾行。过短条目应合并进相邻句、并入同类 bullets，或删去无信息填充，而不是单独占一行。",
...
async def refine_patch_operation(
    ...
    prompt = _prompt(
        ...
        constraints=[
            ...
            "工作/项目经历改写坚持 STAR（情境/任务—行动—结果）；能分点就拆成多条 bullets，不要把不同阶段揉成一段",
            "注意版面：每条正文尽量填满一行主要宽度；避免一行只有几个字，过短内容应合并或基于已有事实写充实一点，不要空疏占行",
