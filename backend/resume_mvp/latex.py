...
def _build_overleaf_latex(resume: ResumeDocument, application_type: ApplicationType) -> str:
    ...
    if resume.basics.photo_data_url:
        lines.extend([
            ...
            f"  \\node[anchor=north east] at ($(current page.north east)+(-0.65cm,-0.45cm)$) {{\\IfFileExists{{{photo_filename}}}{{\\includegraphics[height=2.35cm,keepaspectratio]{{{photo_filename}}}}}{{}}}};",
            "\\end{tikzpicture}",
        ])
    if resume.education:
